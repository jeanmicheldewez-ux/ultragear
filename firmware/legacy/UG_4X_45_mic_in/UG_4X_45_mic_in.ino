...
// Legacy OTA credentials removed for public release.
const char* SSID = "";
const char* PASS = "";  
...
const int chanToLed[8] = { 1,2,3,4,5,6,7,8 };
...
int myPin[14] = {9,10,11,12  ,4,5,6,7,  13,14,    8,3,   1,2};
...
int newNotes[NUM_NOTES] = {0};

String inputBuffer = "";  // accumulate serial characters
...
void loopSerial() {
  while (Serial.available()) {
    ...
    // accumulate until we get a closing '>'
    if (c == '<') {
      inputBuffer = ""; // start of new message
    } else if (c == '>') {
      ...
      for (int i = 0; i < inputBuffer.length(); i++) {
        if (inputBuffer[i] == ',' || i == inputBuffer.length() - 1) {
          int end = (i == inputBuffer.length() - 1) ? i + 1 : i;
          if (index < NUM_NOTES) {
            String numStr = inputBuffer.substring(start, end);
            newNotes[index] = numStr.toInt();
            index++;
          }
          start = i + 1;
        }
      ...
      for (int i = 0; i < NUM_NOTES; i++) {
         scale[0][i] = newNotes[i];  
         scale[1][i] = newNotes[i]+12;           
         scale[2][i] = newNotes[i]+15;          
         scale[3][i] = newNotes[i]+24;                 
      }
      

      inputBuffer = ""; // reset buffer
    } else {
      inputBuffer += c; // keep accumulating
    }
  }
}
